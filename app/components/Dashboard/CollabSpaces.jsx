...
const CollabSpaces = (props) => {
  ...
  const collabSpacesToDisplay = collabSpaces.map((el) => {
    const route = `space/${el._id}`;
    return (
      <div key={el._id} className={`${cx('iconForSheet')} col-xs-4 col-sm-4 col-md-3 col-lg-2`}>
        <Link to={route}>
  ...
  return (
    <div className={cx('spacesDiv')}>
      ...
      <div className={cx('spaces')}>{collabSpacesToDisplay}</div>
    </div>
  );
};
