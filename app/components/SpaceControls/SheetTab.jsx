...
class SheetsTab extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      this.props.sheets &&
      nextProps.sheet &&
      this.props.sheets.length !== nextProps.sheets.length
    )
      this.setState({name: nextProps.sheet});
  }
  ...
  showSheet() {
    ...
    if (!this.active) {
      this.props.dispatch(
        Actions.saveAllSheets(
          this.props.sheets,
          this.props.sheetToShow,
  ...
  render() {
    this.active =
      this.props.sheetToShow && this.props.sheetToShow._id === this.props.sheetId
        ? 'activeSheet'
        : '';
    return (
      <div
        onClick={this.active ? null : this.showSheet}
