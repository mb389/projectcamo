...
export default function spaceControl(state = {}, action = {}) {
  ...
  switch (action.type) {
    ...
    case UPDATE_REF_SHEET: {
      ...
      newState.sheets.filter((sheet) => sheet._id === action.targetSheet._id).forEach((sheet) => {
        ...
        if (existingCol) {
          sheet.content.grid.forEach((row) => {
            if (row['100'].data === action.data.data) {
              row[existingCol.id].data
                ? row[existingCol.id].data.push(newRefLabel)
                : (row[existingCol.id].data = [newRefLabel]);
            }
          });
        } else {
    ...
    case CHANGE_SHEET_NAME: {
      ...
      const sheetNames = state.sheetNames.map(
        (sheetInSpace) =>
          sheetInSpace.id === action.sheetId
            ? {
                name: action.name,
                id: sheetInSpace.id,
              }
